...
def valid_ship_padding(ship, board):
    for x, y in ship:
...
def recursion(ships_positions, densities, redundancy_indices, ship_indices=[], i=0, board=[]):
    ...
    for j in range(redundancy_start, len(ships_positions[i])):
        ...
        if valid_ship_padding(ship, board):
            recursion(ships_positions, densities, redundancy_indices,
                      ship_indices + [j], i+1, board + ship)


recursion(ships_positions, densities, [2, 4, 5, 7, 8, 9])
